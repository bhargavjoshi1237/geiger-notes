...
> **This product's schema is `notes`.** Its migration ledger is
> `notes.geiger_migrations`. Substitute `notes` wherever this doc writes
...
(`notes.tickets`), never `notes.flow_tickets`.
...
-- Add seat maps
--
-- Owns notes.seat_maps. Free-form header: what this changes and why.

-- @up
create table if not exists notes.seat_maps (
...
-- @down
drop table if exists notes.seat_maps;
```
...
create table if not exists notes.tickets (...);
alter table notes.tickets add column if not exists tier text;   -- back-fill older copies
create index if not exists tickets_event_idx on notes.tickets (event_id);

drop trigger if exists tickets_touch_updated_at on notes.tickets;
create trigger tickets_touch_updated_at
before update on notes.tickets
for each row execute function notes.touch_updated_at();

drop policy if exists tickets_demo_all on notes.tickets;
create policy tickets_demo_all on notes.tickets for all to anon, authenticated
...
```sql
alter table notes.tickets enable row level security;
drop policy if exists tickets_demo_all on notes.tickets;
create policy tickets_demo_all on notes.tickets for all to anon, authenticated
  using (true) with check (true);
```
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on notes.tickets (event_id);
```
