...
export const useHomeLogic = (id, boardId) => {
    ...
    const saveOnUnload = useCallback(() => {
        ...
        const payload = JSON.stringify({ userId: id, nodes, edges, viewport, boardId });
        try {
            navigator.sendBeacon(`${basePath}/api/save-state`, new Blob([payload], { type: 'application/json' }));
        } catch {
            // sendBeacon failed (e.g. payload too large), try keepalive fetch
    ...
    }, [id, boardId, isInitialized, saveOnUnload]);
